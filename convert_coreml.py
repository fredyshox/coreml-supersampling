...
def main(args):
    ...
    build_model_load_weights(model, args.weights_path, args.frame_count, args.input_shape)
    
    output_path_no_ext, output_ext = os.path.splitext(args.output_path)
    if output_ext != "mlpackage":
        output_path = output_path_no_ext + ".mlpackage"
    else:
...
def build_model_load_weights(model, weight_path, frame_count, shape):
    dummy_data = {
    ...
    if weight_path is not None:
        weights_file_path = resolve_weights_uri(weight_path)
...
def parse_args():
    ...
    parser.add_argument("--full", action="store_true",  help="Convert full model, instead of only reconstruction module")
    # model config
